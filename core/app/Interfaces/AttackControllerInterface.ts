...
export default interface AttackControllerInterface {
    ...
    attackTo(target: DamageControllerInterface, afterDiedTargetOptions?: EnemyRewardOptions): boolean;   //todo: Надо как-то придумать названия для генерации УРОНА и получение урона. А то сейчас оба метода damage() и damage(damage)
    canAttack(): boolean;
}