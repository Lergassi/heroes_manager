...
export class Farming implements FarmingRenderInterface {
    ...
    buildGardenBed(itemStorage: ItemStorageInterface, wallet: WalletInterface): boolean {
        if (this._gardenBeds.length >= this._options.max) {
            DebugApp.debug(DebugNamespaceID.Throw)('Кол-во построек достигло максимума.');
            return false;
        }
        ...
        if (!itemStorage.hasItems(data.requireItems)) {
            DebugApp.debug(DebugNamespaceID.Throw)('Не достаточно ресурсов для строительства.');
            return false;
        }
        ...
        DebugApp.debug(DebugNamespaceID.Log)('Грядка построена.');
