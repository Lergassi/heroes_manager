...
export default class LoadFullEnvironmentCommand extends Command {
    ...
    async execute(input: Input) {
        ...
        await this.container.get<GameConsole>('gameConsole').run('load_user_env', [usedID]);
        await this.container.get<GameConsole>('gameConsole').run('load_player_env', [playerID]);
    }
}