...
function serializeRealEntities(container: ContainerInterface, serializer: Serializer, jsonSerializer: JsonSerializer) {
    ...
    // debugItem(entityManager.getRepository<Item>(Item.name).getOneByAlias('item_wood'));
    // debugItem(entityManager.getRepository<Item>(Item.name).getOneByAlias('item_iron_ore'));
    // debugItem(entityManager.getRepository<Item>(Item.name).getOneByAlias('item_one_handed_sword_01'));
    // debugItem(entityManager.getRepository<Item>(Item.name).getOneByAlias('item_plate_breastplate_01'));
}
...
export function testSerializeHero(container: ContainerInterface, serializer: Serializer, jsonSerializer: JsonSerializer) {
    ...
    let hero = container.get<HeroFactory>('player.heroFactory').create(heroClass);
    debugHero(hero);
