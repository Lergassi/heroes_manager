...
export const generate_items_by_patterns = (container: ContainerInterface, options?: {
    ...
    DebugApp.debug(DebugNamespaceID.Log)(sprintf('Алгоритм by_pattern, v%s.', '0.0.1'));
    ...
    if (options?.onlyGenerate) {
        DebugApp.debug(DebugNamespaceID.Log)('Данные сгенерированы, но не записаны. Флаг onlyGenerate=true');
        return;
    }
    ...
    fs.writeFile(itemsPathname, itemsString, (error) => {
        ...
        DebugApp.debug(DebugNamespaceID.Log)(sprintf('Данные записаны в файл: %s', itemsPathname));
        DebugApp.debug(DebugNamespaceID.Log)(sprintf('Предметов создано: %s.', itemsCount));
    });
    ...
    fs.writeFile(recipesPathname, recipesString, (error) => {
        ...
        DebugApp.debug(DebugNamespaceID.Log)(sprintf('Рецепты записаны в файл: %s', recipesPathname));
        DebugApp.debug(DebugNamespaceID.Log)(sprintf('Рецепотов создано: %s.', itemsCount));
    });
    ...
    fs.writeFile(enumItemIDStringPathname, enumIDsString, (error) => {
        ...
        DebugApp.debug(DebugNamespaceID.Log)(sprintf('__ItemAutoGeneratedID created: %s', enumItemIDStringPathname));
    });
    ...
    for (let i = 0; i < autoGeneratedContent.length; i++) {
        fs.writeFile(autoGeneratedContent[i].filepath, autoGeneratedContent[i].content, (error) => {
            ...
            DebugApp.debug(DebugNamespaceID.Log)(sprintf('autogenerated code created: %s', autoGeneratedContent[i].filepath));
        });
    }
}